...
3. Confirma a direção do PR:
   - **base repository:** `cunhaestgv/AI3-2026-2027` · **base:** `master`
...
| Obter novos ficheiros do repositório do docente | `git fetch upstream && git merge upstream/master` |
...
Move o trabalho para uma branch nova (assumindo que ainda não foi feito *push* da `master`):

```bash
git checkout -b tarefa-1         # leva os commits para a nova branch
git checkout master
git fetch upstream               # necessário para o passo seguinte
git reset --hard upstream/master   # repõe a master como está no docente
```

> ⚠️ O `reset --hard` descarta definitivamente tudo o que estiver na `master` e não tiver sido levado para a nova branch, incluindo alterações por confirmar. Confirma primeiro com `git log --oneline` que os commits já estão em `tarefa-1`.
</details>
...
Fecha o Pull Request e abre um novo, confirmando que **base repository** é `cunhaestgv/AI3-2026-2027` e **base** é `master`.
</details>
